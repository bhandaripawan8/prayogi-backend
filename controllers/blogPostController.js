...
export const createBlogPost = async (req, res) =>{
    try{
        ...
        const existingBlog = blogPost.findOne({header, user});
        if(existingBlog){
            return res.status(400).res({message: 'Duplicate blog post found, please use a different header'})
            console.log('Duplicate blog post found, please use a different header')
        }
        const newPost = new BlogPost({
            image, header, user, paragraph
        });
