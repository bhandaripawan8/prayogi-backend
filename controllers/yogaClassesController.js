...
export const createNewClass = async (req, res) => {
    try {
        ...
        const existingClass = await YogaClasses.findOne({ header });
        if (existingClass) {
...
export const updateYogaClass = async (req, res) => {
    try {
        const { image, header, description, days, time, user } = req.body;
        const updatedClass = await YogaClasses.findByIdAndUpdate(req.params.id, { image, header, description, days, time, user }, { new: true });
        if (updatedClass) {
